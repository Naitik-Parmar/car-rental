...
import React, { useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import { Card, Button, Image, Modal } from 'semantic-ui-react'; // Import Modal from Semantic UI
import axios from 'axios';
import './menu.css';
import { useNavigate } from 'react-router-dom';
...
const UpdateCarForm = ({ car, setCars, setOpenModal }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8080/api/admin/car/${car.id}`, updatedFields, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
